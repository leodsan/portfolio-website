      ...
      {items.map(({ id, title, link, date, subtitle }) => (
        ...
        >
          <Button target type="a" href={link} className="w-full pb-14 gap-9">
            <div className="flex items-center justify-between flex-grow tp-h1 md:flex-col md:items-start gap-3">
              <div className="flex gap-5 items-end lg:flex-col lg:items-start lg:gap-2">
                <span>{title}</span>
                <span className="lg:hidden">_</span>
                <span className="tp-h4">{subtitle}</span>
              </div>
              <span className="md:!tp-h4">{date}</span>
            </div>
          </Button>
        </li>
      ))}
